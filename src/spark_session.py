...
def create_spark_session(
    ...
    try:
        ...
        # Keep Spark scheduler noise out of the console unless a real error occurs
        spark.sparkContext.setLogLevel("ERROR")

        try:
            log4j = spark._jvm.org.apache.logging.log4j
            configurator = log4j.core.config.Configurator
            configurator.setLevel("org.apache.spark.scheduler.DAGScheduler", log4j.Level.ERROR)
            configurator.setLevel("org.apache.spark.scheduler.TaskSetManager", log4j.Level.ERROR)
            configurator.setLevel("org.apache.spark.sql.execution.QueryExecution", log4j.Level.ERROR)
        except Exception:
            # Best-effort only; Spark still runs even if logger tuning is unavailable.
            pass
