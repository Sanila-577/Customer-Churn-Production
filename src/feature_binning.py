...
class CustomBinningStrategy(FeatureBinningStrategy):
    ...
    def bin_feature(self, df: DataFrame, column: str) -> DataFrame:
        ...
        if column == "tenure":
            bin_column = "Tenure_category"
        elif column == "MonthlyCharges":
            bin_column = "Charge_category"
        else:
            bin_column = f"{column}Bins"

        if column == "MonthlyCharges":
            lower_quantile, upper_quantile = df.approxQuantile(column, [0.33, 0.66], 0.01)
            df_binned = df.withColumn(
                bin_column,
                F.when(F.col(column) <= lower_quantile, "Low")
                .when(F.col(column) <= upper_quantile, "Medium")
                .otherwise("High")
            )
            ...
            logger.info(f"✓ Added '{bin_column}' column based on monthly charge tertiles")
            logger.info(f"{'='*60}\n")
            return df_binned
        ...
        # IMPORTANT: Keep original column alongside binned column for model compatibility
        # df_binned = df_binned.drop(column)  # DO NOT DROP - model expects both columns
    ...
    def service_count(self, df: DataFrame, service_columns: List[str]) -> DataFrame:
        """Add a Service_count column based on how many listed services equal 'Yes'."""
        expressions = [
        ...
        if not expressions:
            return df.withColumn("Service_count", F.lit(0))

        count_expr = reduce(lambda left, right: left + right, expressions)
        return df.withColumn("Service_count", count_expr)

    def bundle_user(self, df: DataFrame, bundle_columns: List[str]) -> DataFrame:
        """Add a Bundle_user column for customers with both phone and internet service."""
        if "InternetService" not in df.columns or "PhoneService" not in df.columns:
            return df

        return df.withColumn(
            "Bundle_user",
            F.when((F.col("InternetService") != "No") & (F.col("PhoneService") == "Yes"), 1).otherwise(0)
        )
