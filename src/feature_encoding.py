...
class NominalEncodingStrategy(FeatureEncodingStrategy):
    ...
    def __init__(self, nominal_columns: List[str], one_hot: bool = True, spark: Optional[SparkSession] = None):
        ...
        self.one_hot = one_hot  # Default to True for one-hot encoding
        self.encoder_dicts = {}
        self.indexers = {}
        self.encoders = {}
    ...
    def encode(self, df: DataFrame) -> DataFrame:
        ...
        for column in self.nominal_columns:
            ...
            # Save encoder dictionary with encoding type
            encoder_path = os.path.join('artifacts/encode', f"{column}_encoder.json")
            encoder_data = {
                'categories': labels,
                'encoding_type': 'one_hot' if self.one_hot else 'label',
            ...
            if self.one_hot:
                ...
                # Create UDF to extract vector elements
                def extract_vector_element(idx):
                    def get_element(vec):
                        try:
                            return float(vec[idx]) if vec and idx < len(vec) else 0.0
                        except:
                            return 0.0
                    return udf(get_element, DoubleType())
                ...
                for idx, label in enumerate(labels):
                    ...
                    df_encoded = df_encoded.withColumn(
                        col_name,
                        extract_vector_element(idx)(col(f"{column}_vec"))
                    )
                    logger.info(f"    • Created binary column: {col_name}")
                
                # Drop intermediate columns
                df_encoded = df_encoded.drop(column, f"{column}_index", f"{column}_vec")
                logger.info(f"  ✓ Applied one-hot encoding to column '{column}' → {len(labels)} binary columns")
            else:
