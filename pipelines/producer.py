...
class CustomerEventGenerator:
    ...
    def __init__(self, seed: int = 42):
        ...
        data_path = os.path.join(project_root, 'data/raw/TelcoCustomerChurn.csv')
...
class MLKafkaProducer:
    ...
    def produce_stream(self, topic: str = 'churn_predictions', 
        ...
        try:
            ...
            logger.info("Streaming stopped by producer")
            return successful
...
def main():
    ...
    # Resolve topic from config if not provided
    if args.topic is None:
        args.topic = load_config().get('kafka', {}).get('producer', {}).get('default_topic', 'telco.raw.customers')
