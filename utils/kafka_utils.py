...
def create_topic(topic_name: str, partitions: int = 1, replication_factor: int = 1) -> bool:
    ...
    try:
        ...
        # Use configured bootstrap servers if available
        bootstrap = load_config().get('kafka', {}).get('bootstrap_servers', 'localhost:9092')
...
def setup_ml_topics() -> bool:
    ...
    try:
        ...
        # Define topics from configuration (fallback to PDF naming)
        cfg = load_config().get('kafka', {})
        topics = [
            cfg.get('topics', {}).get('raw_input', 'telco.raw.customers'),
            cfg.get('topics', {}).get('predictions_output', 'telco.churn.predictions'),
            cfg.get('topics', {}).get('deadletter', 'telco.deadletter')
        ]
